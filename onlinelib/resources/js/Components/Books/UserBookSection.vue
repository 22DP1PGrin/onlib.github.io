<script setup>
    ...
    const visibleBooks = computed(() => {
        ...
        return props.books.slice(0, currentLimit.value);
    });

    // Vai ir vēl grāmatas, ko ielādēt
    const hasMoreBooks = computed(() => {
        return currentLimit.value < props.books.length;
    });
    ...
    const loadMore = () => {
        ...
        if (newLimit >= props.books.length) {
            // Ja jaunais limits pārsniedz kopējo grāmatu skaitu, parāda visas
            currentLimit.value = props.books.length;
            showAll.value = true;
        } else {
    ...
    // Funkcija slēpj visas papildus ielādētās grāmatas (atgriežas pie sākotnējā limita)
    const collapse = () => {
...
<template>
    ...
    <section class="book-section" v-if="books.length">
        ...
        <div class="books-grid">
            ...
            <div v-for="book in visibleBooks" :key="book.id" class="book-card">
                <div class="book-content">
                    ...
                    <!-- Autora informācijas bloks (lietotājvārds) -->
                    <div class="book-info">
                        <span class="book"><strong>Autors: </strong>{{ book.user.nickname }}</span>
                    </div>

                    <!-- Statusa informācijas bloks -->
                    <div class="book-info">
                        <span class="book"><strong>Statuss: </strong>{{ book.status }}</span>
                    </div>
        ...
        <div class="toggle-container" v-if="books.length > initialLimit">
            <button
